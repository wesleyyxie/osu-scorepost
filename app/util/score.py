...
class ScoreInfo:
    ...
    def __init__(
        ...
        if score_ossapi != None:
            ...
            self.score = score_ossapi.legacy_total_score or score_ossapi.total_score
            self.beatmapset_status = score_ossapi.beatmapset.status.value

            self.count_300 = score_ossapi.statistics.great or 0
            self.count_100 = score_ossapi.statistics.ok or 0
            self.count_50 = score_ossapi.statistics.meh or 0
            self.count_miss = score_ossapi.statistics.miss or 0
            self.rank = score_ossapi.rank.value
            self.created_at = f'{score_ossapi.ended_at.strftime("%d.%m.%Y %H:%M:%S")}.'
        else:
